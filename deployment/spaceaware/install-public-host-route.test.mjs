...
test('installs host-aware SDN routing without changing the SpaceAware backend', async (t) => {
  ...
  assert.match(installed, /map \$http_upgrade \$sdn_upgrade_backend \{\s+default \$sdn_http_backend;\s+websocket http:\/\/127\.0\.0\.1:18080;\s+\}/);
  ...
  assert.match(installed, /location = \/index\.html \{\s+proxy_pass \$sdn_http_backend;\s+\}/);
...
test('the SpaceAware deploy uses the reviewed installer only for the exact production config', async () => {
  ...
  assert.match(
    deploy,
    /configure_spaceaware_public_host_route\(\) \{[\s\S]*if ! is_spaceaware_config; then\s+return\s+fi[\s\S]*scp_cmd "\$\{DEPLOY_DIR\}\/spaceaware\/install-public-host-route\.mjs"[\s\S]*node \/opt\/spacedatanetwork\/deployment\/spaceaware\/install-public-host-route\.mjs/,
  );

# Wiggum - Custom IPFS Fork for Space Data Network

## Overview

**Wiggum** is a specialized fork of IPFS (Kubo server + IPFS Desktop) tailored for the Space Data Network. It replaces generic content-addressed storage with FlatBuffer-native data handling and SQLite-based structured storage, optimized for space data standards.
...
**Remove:**
- [ ] File browser UI (replaced by SDS data explorer)
- [ ] IPFS config UI (simplified for Wiggum)
...
- [ ] Create `edge-relays.cpp` WASM module with embedded encryption
- [ ] Build script to generate encrypted relay data
- [ ] DHT monitor service (`registry-builder`)
- [ ] CDN deployment automation
- [ ] Subresource Integrity (SRI) hash generation
- [ ] WASM signature verification in JS loader
- [ ] Fallback relay list for offline/failure scenarios
...
- [ ] Copy `internal/node/protocols/` → `spacedatanetwork-server/internal/protocol/`
- [ ] Copy `internal/node/sds_utils/` → `spacedatanetwork-server/internal/sds/`
- [ ] Copy `internal/spacedatastandards/` → `spacedatanetwork-server/internal/schemas/`
- [ ] Copy `javascript/sdn.libp2p.ts` → `spacedatanetwork-js/src/legacy/`

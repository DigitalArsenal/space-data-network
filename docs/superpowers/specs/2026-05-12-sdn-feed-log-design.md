...
- CelesTrak OMM is expected to meet the 80% target only when the immutable shard CIDs are seeded by both CelesTrak and SpaceAware. A single-provider fresh-cache download previously topped out at 41-54% of measured wire speed; two-seed runs must be reported against a fresh, immediately preceding wire-speed probe and utilization must be bounded to 100% so cache effects or stale probes cannot appear faster than the measured path.
...
- Desktop/browser sync starts IPFS provider discovery for published shard CIDs as a bounded background warmup while configured artifact seed peers are connected before gateway shard reads, so DHT latency cannot block the first resumable FlatSQL shard download.
...
- `open_manifest` can advertise verified shard-group CAR bundles from the pin ledger for provider-side seeding and external cache warmup. Browser/desktop interactive sync does not import the full CAR bundle on the download path; it fetches the ordered missing FlatSQL shard CIDs directly so resumable sync can show row progress immediately.

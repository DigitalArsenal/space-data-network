...
- [x] Run the Desktop package command documented in `desktop/package.json`.
  - 2026-06-24: `npm --prefix desktop run package` completed successfully and
    produced `desktop/dist/space-data-network-desktop-0.47.0-mac.dmg`,
    `desktop/dist/space-data-network-desktop-0.47.0-squirrel.zip`, blockmaps,
    and `desktop/dist/mac-universal/Space Data Network.app`. Electron Builder
    skipped macOS signing because no valid Developer ID identity is installed.
- [x] Reinstall or refresh the local installed app.
  - 2026-06-24: Quit the running app, confirmed it stopped, then refreshed
    `/Applications/Space Data Network.app` from
    `desktop/dist/mac-universal/Space Data Network.app`; installed bundle reports
    `CFBundleShortVersionString` `0.47.0`.
- [x] Restart the local Desktop app.
  - 2026-06-24: `open -a "Space Data Network"` started the refreshed app.
    Verification observed app PID `1387` and bundled Kubo daemon PID `1540`.
- [x] Record the exact command output or blocker in the completion report.
  - 2026-06-24: No blocker. Packaging, refresh, and restart completed; signing

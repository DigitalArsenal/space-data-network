...
Options:
  ...
  --artifact-private-key-file <path>
...
read_hex_file() {
  ...
  [[ -f "$file_path" ]] || fail "private key file not found: $file_path"
  local value
  value="$(<"$file_path")"
  normalize_hex "$value"
}
...
if [[ -n "${PLUGIN_KEY_SERVER_ARTIFACT_PRIVATE_KEY_HEX:-}" ]]; then
  fail "PLUGIN_KEY_SERVER_ARTIFACT_PRIVATE_KEY_HEX is forbidden. Use --artifact-private-key-file."
fi
if [[ -n "${PLUGIN_SERVER_PRIVATE_KEY_HEX:-}" ]]; then
  fail "PLUGIN_SERVER_PRIVATE_KEY_HEX is forbidden. Use --server-private-key-file."
fi
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --artifact-dir)
      ...
      ARTIFACT_DIR=$1
      ;;
    --artifact-private-key-file)
      shift
      [[ $# -gt 0 ]] || fail "--artifact-private-key-file requires a value"
      PRIVATE_KEY_FILE=$1
      ;;
    --server-private-key-file)
      shift
      [[ $# -gt 0 ]] || fail "--server-private-key-file requires a value"
      SERVER_PRIVATE_KEY_FILE=$1
      ;;
    --skip-build)
      SKIP_BUILD=true
      ;;
...
if [[ -n "$PRIVATE_KEY_FILE" ]]; then
  PRIVATE_KEY="$(read_hex_file "$PRIVATE_KEY_FILE")"
fi
if [[ -n "$SERVER_PRIVATE_KEY_FILE" ]]; then
  SERVER_PRIVATE_KEY="$(read_hex_file "$SERVER_PRIVATE_KEY_FILE")"
fi

if [[ "$SKIP_BUILD" == "true" && ( -z "$PUBLIC_KEY" || -z "$PRIVATE_KEY" ) ]]; then
  fail "--skip-build requires PLUGIN_KEY_SERVER_ARTIFACT_PUBLIC_KEY_HEX and --artifact-private-key-file"
fi
...
NODE
  ...
  fail "public key is set but private key is not; provide --artifact-private-key-file when local decryption is needed"
fi
...
if [[ -z "$SERVER_PRIVATE_KEY_FILE" ]]; then
  SERVER_PRIVATE_KEY_FILE="$WORKSPACE_DIR/server-private-key.hex"
  printf '%s\n' "$SERVER_PRIVATE_KEY" > "$SERVER_PRIVATE_KEY_FILE"
  chmod 600 "$SERVER_PRIVATE_KEY_FILE" 2>/dev/null || true
fi
...
(
  env \
    ...
    PLUGIN_SERVER_PRIVATE_KEY_FILE="$SERVER_PRIVATE_KEY_FILE" \

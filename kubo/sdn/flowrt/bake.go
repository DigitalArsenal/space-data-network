...
func (b *Baker) resolve(ctx context.Context, g *bakeGraph, refs []BakeModuleRef) (*generated, []depObject, bool, error) {
	...
	for i, n := range g.Nodes {
		...
		symbols[i] = sym
		// Phase-2 typed input ports for the node's bound method (staged from the
		// module's plugin-manifest.json). These drive the required-ports readiness
		// gate below: a multi-input node fires only when ALL of them have a frame.
		inputPorts[i] = methodInputPortIDs(meta, n.MethodID)
...
// a staged module's metadata (Phase-2 plugin-manifest.json enrichment), or nil
// when the module was staged without a manifest / the method declares no typed
// input ports. Order follows the manifest.
func methodInputPortIDs(meta *flowcc.ModuleMetadata, methodID string) []string {
	if meta == nil {
		return nil
	}
	for _, mp := range meta.MethodPorts {
		if mp.MethodID != methodID {
			continue
		}
		ids := make([]string, 0, len(mp.InputPorts))
...
func generateDescriptorSources(g *bakeGraph, nodeIndex map[string]int, symbols []string, inputPorts [][]string, depCount int) (*generated, error) {
	n := len(g.Nodes)
	...
	// Required-ports readiness rows. A node is READY only when EVERY one of its
	// required input ports has a queued frame (flow_runtime.cpp
	// flow_node_is_ready). We derive the required set per node from the bound
	// method's typed input ports (inputPorts[i], staged from plugin-manifest.json)
	// INTERSECTED with the ports actually wired into the node (an incoming edge's
	// toPortId or a trigger binding's targetPortId). The intersection is what
	// makes this correct AND deadlock-free: we only require a port that both the
	// method declares as an input AND the flow author wired, so a frame will
	// actually arrive on it. A node with no typed input ports (untyped module) or
	// no wired inputs emits no rows and keeps the fire-on-any-frame behavior.
	requiredRows := computeRequiredPorts(g, nodeIndex, inputPorts)
	requiredPortCount := len(requiredRows)
	...
	// Required ports: one row per (node, required-input-port). A node with two
	// required rows fires only after BOTH ports have a queued frame. A dummy row
	// keeps the array non-zero-length when there are none.
	fmt.Fprintf(&b, "static FlowRequiredPort g_required_ports[%du%s] = {\n", requiredPortCount, plusOneIfZero(requiredPortCount))
	for _, rp := range requiredRows {
		fmt.Fprintf(&b, "  { %du, %s },\n", rp.node, cQuote(rp.port))
	}
...
// requiredPort is one emitted FlowRequiredPort row: a node index + the input
// port id that must have a queued frame before the node is ready.
type requiredPort struct {
	node int
	port string
}

// computeRequiredPorts derives the per-node required-input-port rows for the
// readiness gate. For each node it takes the intersection of the node's typed
// input ports (inputPorts[i], from the staged manifest) and the ports actually
// wired into it (edge toPortId + trigger-binding targetPortId). Only wired,
// declared inputs are required — so a required port is always one a frame can
// reach (no deadlock), and a multi-input node waits for all of them. Rows are
// emitted in node order then declared-port order for a deterministic descriptor.
func computeRequiredPorts(g *bakeGraph, nodeIndex map[string]int, inputPorts [][]string) []requiredPort {
	// Ports actually wired into each node index.
	wired := make([]map[string]bool, len(g.Nodes))
	markWired := func(nodeID, portID string) {
		if portID == "" {
			return
		}
		...
		if wired[idx] == nil {
			wired[idx] = map[string]bool{}
		}
		wired[idx][portID] = true
	}
	for _, e := range g.Edges {
		markWired(e.ToNodeID, e.ToPortID)
	}
	for _, tb := range g.TriggerBindings {
		markWired(tb.TargetNodeID, tb.TargetPortID)
	}

	var rows []requiredPort
	for i := range g.Nodes {
		if len(wired[i]) == 0 || i >= len(inputPorts) || len(inputPorts[i]) == 0 {
			continue
		}
		seen := map[string]bool{}
		for _, port := range inputPorts[i] {
			if port == "" || seen[port] || !wired[i][port] {
				continue
			}
			seen[port] = true

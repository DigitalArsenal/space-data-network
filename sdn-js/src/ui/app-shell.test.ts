...
describe('renderAppShell', () => {
  it('renders the admin shell workspaces and top-level controls without a custom account dialog', async () => {
    ...
    expect(root.innerHTML).toContain('>Peers<');
    expect(root.innerHTML).not.toContain('Connection status');
    expect(root.innerHTML).not.toContain('id="sdn-connection-status"');
    expect(root.innerHTML).not.toContain('id="sdn-connection-status-top"');
    expect(root.innerHTML).toContain('Space Data Network combines SDS records, libp2p and IPFS transport');
    expect(root.innerHTML).toContain('id="sdn-feature-carousel"');
    expect(root.innerHTML).toContain('>Space Data Network<');
    ...
    expect(root.innerHTML).toContain('>Search SDN Peers<');
    expect(root.innerHTML).toContain('placeholder="EPM handle, bitcoin address, peer ID, or xpub"');

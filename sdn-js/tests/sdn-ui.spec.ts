...
test('data route renders subscribed local datastore preview without workbench status chrome', async ({ page }) => {
  ...
  await expect(syncSettings.getByRole('button', { name: 'Active' })).toHaveCount(0);
  await expect(syncSettings.getByRole('button', { name: 'Trials' })).toHaveCount(0);
  await expect(syncSettings.getByRole('button', { name: 'Payment issues' })).toHaveCount(0);
  const subscriptionFilterButton = syncSettings.getByRole('button', { name: 'Filters' });
  await expect(subscriptionFilterButton).toBeVisible();
  ...
  await subscriptionFilterButton.click();
  const subscriptionFilterMenu = page.getByLabel('Subscription filter options');
  await expect(subscriptionFilterMenu.getByRole('checkbox', { name: 'Free' })).toBeVisible();

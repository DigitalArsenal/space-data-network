...
const pickTitle = (t, ipfsReady, ipfsConnected) => {
  if (ipfsReady && ipfsConnected) return 'Connected to the Space Data Network'
  if (ipfsReady) return t('ipfsApiRequestFailed')
  return t('app:status.connectingToIpfs')
}

/**
 * SDN root keeps the upstream connected widget behavior and only swaps the
 * connected label shown in the top chrome.
 *
 * Synced base: webui/src/components/connected/Connected.js
 */
export const Connected = ({ t, className, size = 28, ipfsReady, ipfsConnected, doSetIsNodeInfoOpen }) => {
  const title = pickTitle(t, ipfsReady, ipfsConnected)

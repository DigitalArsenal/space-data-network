...
func (h *Handler) handleMe(w http.ResponseWriter, r *http.Request) {
	...
	// failure — the session already proved itself (the root ceremony mints
	// exactly such rowless sessions; me_session_tier_test.go pins this).
	user, _ := h.userStore.GetUser(session.XPub)
	...
	writeJSON(w, http.StatusOK, h.meBody(session, user))
}
...
// meBody projects the caller's session — and, when one exists, their operator
// row — into the account-screen response. Identity and tier ALWAYS come from
// the session: the fingerprint is the session key's, and the trust level is
// the value every RequireTrust gate enforces, never the row's
// (me_session_tier_test.go). The row contributes display metadata only, and
// its absence (a root-ceremony session) still yields a complete answer.
func (h *Handler) meBody(session *Session, user *User) meResponse {
	body := meResponse{
	...
	body.Notes = user.Notes
	body.VCardData = user.VCardData
	body.Source = user.Source
	body.SigningPubKeyHex = user.SigningPubKeyHex
	body.SignInCount = user.SignInCount
	body.Editable = user.Source != "config"

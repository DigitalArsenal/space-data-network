...
var unguardedEmbeddedSchemas = map[string]bool{
	// $TRP / $TRV (v1.207.0 mint, dashboard-console program): the host
	// decodes both in internal/trust with the pinned bindings and round-trips
	// them in trust tests (rules_test.go), which is the compensating control
	// until they graduate to driftGuardedSchemas. $ICN and $DSS graduated with
	// the v1.210.0 pin: the connectors lane decodes $ICN and POST /api/v1/sync
	// decodes $DSS (RETENTION rides on Subscribe) in Go.
	"TRP.fbs":  true,
	"TRV.fbs":  true,
	"ACI.fbs":  true,
	"ACL.fbs":  true,
